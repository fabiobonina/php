<?php
	...
	class CilindroControl extends GlobalControl {
		...
		public function listLoja( $loja ){
			...
			$cilindros	= new Equipamento();
			$itens = array();
			foreach($cilindros->findAll() as $key => $value):if($value->loja_id == $loja) {
				$item = $value;
				$item = (array) $this->matrix( $item );
				array_push( $itens, $item );
			}endforeach;
		...
		public function listLocal( $local ){
			...
			$cilindros	= new Equipamento();
			$itens = array();
			foreach($cilindros->findAll() as $key => $value):if($value->local_id == $local) {
				$item = $value;
				$item = (array) $this->matrix( $item );
				array_push( $itens, $item );
			}endforeach;
		...
		public function contEquipLocal( $local_id ){
			...
			$res['equipQt'] = $cilindros->contLocal( $local_id );
			return $res;
		...
		public function contEquipLoja( $loja_id ){
			...
			$res['equipLoja_tt'] = $cilindros->contLoja( $loja_id );
			return $res;
