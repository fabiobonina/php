...
<html>
  ...
  <button @click='addUluru'>add Uluru</button>
  <h2 v-if="noVisibleMarkers">There are no stores in the current view. Listing stores in 400km radius from your location:</h2>
  <ul class="" id="results-list" v-if="currentZoom > zoomTreshold">
    <li v-for="(marker, i) in visibleMarkers">
      <strong>{{ marker.name }}</strong><br/>
      <span>is boutique: {{ marker.boutique ? 'yes' : 'no'}}</span><br/>
  ...
  <h2 v-else>Your viewport contains {{ visibleMarkers.length }} stores, if you want to show specific stores, please zoom in or type your request into the search field.</h2>
</div>
