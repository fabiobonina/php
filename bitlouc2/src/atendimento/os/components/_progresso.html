<template id="progresso-os">
  <div>
    <v-layout row wrap align-center >
      <v-flex xs12 sm3>
        <v-btn block :small="data != 0" b :class="data >= 0 ?
            data == 0 ? 'green accent-4 white--text' : 'blue white--text small'   
            : 'small light' ">
          <v-icon v-if="data == 0" >mdi-loading mdi-spin</v-icon>
          <v-icon v-else small>mdi-arrow-right-bold</v-icon> Pedente
          <v-icon>mdi-chevron-right</v-icon>
        </v-btn>
      </v-flex>
      <v-flex xs12 sm3>
        <v-btn block :small="data != 1" b :class="data >= 1 ?
            data == 1 ? 'green accent-4 white--text' : 'small blue white--text'   
            : 'small light' ">
      ...
      <v-flex xs12 sm3>
        <v-btn block :small="data != 2" b :class="data >= 2 ?
          ...
          <v-icon v-else small>mdi-redo-variant</v-icon> Encerrado
          <v-icon>mdi-chevron-right</v-icon>
        </v-btn>
      </v-flex>
      <v-flex xs12 sm3>
